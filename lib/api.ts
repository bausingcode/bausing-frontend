...
export async function getAllReviews(params?: {
  ...
  const url = typeof window === "undefined"
    ? `${BACKEND_URL}/admin/reviews?page=${page}&per_page=${per_page}`
    : `/api/admin/reviews?page=${page}&per_page=${per_page}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(error.error || `Failed to get reviews: ${response.statusText}`);
  }
  
  const data = await response.json();
  if (!data.success) {
    throw new Error(data.error || "Failed to get reviews: Invalid response");
  }
 ...
 */
export async function updateReviewStatus(reviewId: string, status: 'published' | 'hidden'): Promise<ProductReview> {
  const url = typeof window === "undefined"
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new Error(error.error || `Failed to update review status: ${response.statusText}`);
  }
  
  const data = await response.json();
  if (!data.success || !data.data) {
    throw new Error(data.error || "Failed to update review status: Invalid response");
  }
...
/**
 * Upload an email image file directly to Supabase email-images bucket
 */
export async function uploadEmailImageFile(file: File): Promise<string> {
  ...
  const fileExt = compressedFile.name.split(".").pop();
  const fileName = `${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExt}`;
  const uploadResponse = await fetch(`${supabaseUrl}/storage/v1/object/email-images/${fileName}`, {
    method: "POST",
    headers: {
      "Authorization": `Bearer ${supabaseKey}`,
      "Content-Type": compressedFile.type,
  ...
  if (!uploadResponse.ok) {
    const errorText = await uploadResponse.text();
    let errorMessage = `Failed to upload: ${uploadResponse.statusText}`;
    try {
      const errorJson = JSON.parse(errorText);
      errorMessage = errorJson.message || errorJson.error || errorMessage;
    } catch {}
  ...
  return `${supabaseUrl}/storage/v1/object/public/email-images/${fileName}`;
}
