...
export default function TransporteTercerizadoPage() {
  ...
  if (isLoading) {
    return <Loader message="Cargando transporte tercerizado..." />;
  }
  ...
  return (
    <div>
      ...
      <div className="mt-6">
        {zones.filter((zone) => getLocalitiesForZone(zone.crm_zone_id).length > 0).length === 0 ? (
          <div className="bg-white border border-gray-200 rounded-[14px] p-8 text-center shadow-sm">
            ...
            <p className="text-gray-600">No hay localidades con transporte tercerizado</p>
          </div>
        ) : (
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="px-2 py-0.5 bg-orange-100 text-orange-700 text-xs font-medium rounded-full shadow-sm">
                          {localities.length} tercerizado{localities.length !== 1 ? "s" : ""}
                        </span>
                      </div>
                    </button>
                        ...
                        <div className="divide-y divide-gray-100">
                          {localities.map((locality) => {
                            ...
                            return (
                              <div
                                key={locality.id}
                                className="p-3.5 bg-gradient-to-r from-orange-50/60 to-orange-50/30 border-l-2 border-orange-300 transition-all duration-200"
                              >
                                {isEditing ? (
