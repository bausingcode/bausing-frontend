                              ...
                              <div className="flex items-center justify-between gap-3">
                                <div className="flex items-center gap-3">
                                  <div className={`w-3 h-3 rounded-full ${
                                    selectedOrder.status === "finalizado" ? "bg-green-600" :
                                    selectedOrder.status === "en reparto" ? "bg-blue-500" :
                                    selectedOrder.status === "pendiente de entrega" ? "bg-amber-500" :
                                    "bg-gray-400"
                                  }`} />
                                  <span className="text-sm font-semibold text-gray-900">
                                    {selectedOrder.status === "pendiente de entrega" && "Pendiente de entrega"}
                                    {selectedOrder.status === "en reparto" && "En reparto"}
                                    {selectedOrder.status === "finalizado" && "Finalizado"}
                                    {selectedOrder.status !== "pendiente de entrega" && 
                                     selectedOrder.status !== "en reparto" && 
                                     selectedOrder.status !== "finalizado" && selectedOrder.status}
                                  </span>
                                </div>
                                <button
                                  onClick={() => {
                            ...
                            >
                              <div className="flex items-start justify-between mb-4">
                                ...
                                <span className={`text-xs font-semibold px-3 py-1 rounded-full ${
                                  ...
                                  order.status === "en reparto" ? "bg-blue-100 text-blue-700" :
                                  order.status === "pendiente de entrega" ? "bg-amber-100 text-amber-700" :
                                  "bg-gray-100 text-gray-700"
                                }`}>
                                  {order.status === "pendiente de entrega" && "Pendiente de entrega"}
                                  {order.status === "en reparto" && "En reparto"}
                                  {order.status === "finalizado" && "Finalizado"}
                                  {order.status !== "pendiente de entrega" && 
                                   order.status !== "en reparto" && 
                                   order.status !== "finalizado" && order.status}
                                </span>
                              </div>
                              <div className="space-y-2">
